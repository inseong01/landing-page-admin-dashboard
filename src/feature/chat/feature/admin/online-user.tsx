...
import type { MessageMetaData } from "../../util/const/chat-const";
...
export default function OnlineUser({
  ...
  return (
    <li
      className={`box-border w-full cursor-pointer p-3 pr-7 pl-7 hover:bg-[#f2f2f2] dark:hover:bg-[#4D4D4D] ${id === selectedID ? "bg-[#f2f2f2] dark:bg-[#4D4D4D]" : ""}`}
      onClick={onClickOpenBtn}
    >
      <div className="mb-1.5 flex items-center justify-between text-[#a7a7a7] max-lg:text-sm">
        {/* 사용자 이름 */}
        <span className="w-3/5 overflow-hidden text-base text-ellipsis whitespace-nowrap text-[#222] dark:text-[#fff]">
          {name}
        </span>
      ...
      <div className="flex h-6 w-full justify-between overflow-hidden leading-5 text-ellipsis whitespace-nowrap max-lg:text-sm">
        {/* 메시지 미리보기 */}
        <span className="w-4/5 max-w-[230px] overflow-x-hidden text-ellipsis whitespace-nowrap text-[#606060] dark:text-[#A7A7A7]">
          {isTyping ? "작성중.." : text}
        </span>

        {/* 안 읽은 메시지 개수 */}
        {messagesLength > 0 && (
          <span className="inline-block rounded-xl bg-[#E83939] p-3 pt-0.5 pb-0.5 font-bold text-white dark:bg-[#CD5656]">
            {count}
          </span>
        )}
      </div>
